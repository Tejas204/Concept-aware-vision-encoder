...
DEFAULT_SPATIAL_RELATIONS = {
    "above", "across", "against", "along", "around", "at", "behind",
    "below", "beneath", "beside", "between", "by", "close to", "in",
    "in front of", "inside", "near", "next to", "on", "on top of",
    "opposite", "outside", "over", "to the left of", "to the right of",
    ...
    with predictions_path.open("w", encoding="utf-8") as output, torch.inference_mode():
        for batch_index, rows in enumerate(tqdm(loader, desc="ARO")):
            ...
            for offset, row in enumerate(rows):
                ...
                true_is_a = random.Random(args.seed + sample_index).random() < 0.5
                caption_a, caption_b = (
